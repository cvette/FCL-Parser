...
exports.DefuzzificationMethods = DefuzzificationMethods;

var OperatorAlgorithms = {
    MAX:  "MAX",
    MIN:  "MIN",
    ASUM: "ASUM",
    PROD: "PROD",
...
exports.OperatorAlgorithms = OperatorAlgorithms;
...
/** EXCEPTIONS */
function AlreadyDefinedException(line, column, symbol) {
    this.message = 'Symbol "' + symbol + '" has already been defined on line ' + line;
...
function ReferenceException(line, column, symbol) {
    this.message = 'ReferenceError: ' + symbol + ' is not defined';
...
function declareSymbol(line, column, symbol, typeNode, scope) {

    // make global default scope
    if (scope === undefined) {
        scope = Scope.FUNCTION_BLOCK;
    }
    ...
    if(symbols[scope][symbol] !== undefined) {
        throw new AlreadyDefinedException(symbols[scope][symbol].line, symbols[scope][symbol].column, symbol);
    }

    var type = typeNode.type;

    if (type === NodeTypes.SIMPLE_SPEC_INIT) {
        type = typeNode.properties.type;
    }

    symbols[scope][symbol] = {type: type, line: line, column: column}
}


function useSymbol(line, column, symbol, scope) {
    // make global default scope
    if (scope === undefined) {
        scope = Scope.FUNCTION_BLOCK;
    }
    ...
    if (symbols[scope][symbol] === undefined) {
        throw new ReferenceException(line, column, symbol);
    }
}
...
exports.FunctionBlock = function (line, column, properties, children) {
    ...
    AstNode.call(this, line, column, properties, children);
    declareSymbol(line, column, properties.name, this, Scope.LIBRARY);

    //clear symbol table except library scope
    symbols[Scope.FUNCTION_BLOCK] = {};
    symbols[Scope.RULE_BLOCKS] = {};
};
...
exports.InputOutputDeclarations = function (line, column, properties, children) {
    this.type = NodeTypes.INPUT_OUTPUT_DECLARATIONS;
    AstNode.call(this, line, column, properties, children);
};
...
exports.RuleBlock = function (line, column, properties, children) {
    ...
    AstNode.call(this, line, column, properties, children);
    declareSymbol(line, column, properties.id, this, Scope.RULE_BLOCKS);
};
...
exports.OperatorDefinition = function (line, column, properties, children) {
    ...
    if ((properties.andMethod === OperatorAlgorithms.MIN && properties.orMethod !== OperatorAlgorithms.MAX)
        || (properties.andMethod === OperatorAlgorithms.PROD && properties.orMethod !== OperatorAlgorithms.ASUM)
        || (properties.andMethod === OperatorAlgorithms.BDIF && properties.orMethod !== OperatorAlgorithms.BSUM)) {
...
exports.WeightingFactor = function (line, column, properties, children) {
    ...
    if(typeof properties.factor === 'string') {
        useSymbol(line, column, properties.factor);
    }
};
...
exports.Subcondition = function (line, column, properties, children) {
    ...
    if(children.length === 1 && typeof children[0] === 'string') {
        useSymbol(line, column, children[0]);
    }
};
...
exports.Equation = function (line, column, properties, children) {
    ...
    useSymbol(line, column, properties.a);
    useSymbol(line, column, properties.b);
};
...
exports.Conclusion = function (line, column, properties, children) {
    ...
    for(var i=0; i < children.length; i++) {
        if(typeof children[i] === 'string') {
            useSymbol(line, column, children[i]);
        }
    }
};
...
exports.LinguisticTerm = function (line, column, properties, children) {
    ...
    declareSymbol(line, column, properties.name, this);
};
...
exports.Point = function (line, column, properties, children) {
    ...
    if (typeof properties.x === 'string') {
        useSymbol(line, column, properties.x);
    }
};
 ...
 * properties: value
 * */
exports.Singleton = function (line, column, properties, children) {
    this.type = NodeTypes.SINGLETON;
    AstNode.call(this, line, column, properties, children);

    if(typeof properties.value === 'string') {
        useSymbol(line, column, properties.value);
    }
};

exports.Singleton.prototype = Object.create(AstNode.prototype);