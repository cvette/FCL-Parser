...
function AST() {
    ...
    var addTerm = function (line, column, symbol, term) {
        ...
        if(terms.indexOf(term) > -1) {
            throw new Error("Term already defined at line " + symbols[self.Scope.FUNCTION_BLOCK][symbol].line);
        }
    ...
    var declareSymbol = function (line, column, symbol, typeNode, scope, variableType) {
        ...
        if(symbols[scope][symbol] !== undefined) {
            throw new Error('Symbol "' + symbol + '" has already been defined on line ' + symbols[scope][symbol].line);
        }
    ...
    var useSymbol = function (line, column, symbol, scope, usageType) {
        ...
        if(symbols[scope][symbol].variableType !== usageType && symbols[scope][symbol].variableType !== self.VariableType.IN_OUT) {
            if(symbols[scope][symbol].variableType === self.VariableType.INPUT) {
                throw new Error('Error: ' + symbol + ' is defined as an input variable but used as output');
            } else {
                throw new Error('Error: ' + symbol + ' is defined as an output variable but used as input');
            }

        }
    };

    var clearSymbols = function (line, column, symbol, scope) {
        symbols = {};
    };
